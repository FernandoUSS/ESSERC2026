...
if __name__ == "__main__":
    ...
    # # Read the Excel file
    # file_path = "metrics_reportedvsaccurate_v2.xlsx"
    # data = pd.read_excel(file_path, header=[0])

    # # filtered_data = data[data["sel"] == "x"]
    # # data = filtered_data
    ...
    if 1: # IdVgs Hysteresis
        ...
        axins.axhline(df['Ith'].iloc[0]/width*1e6, linestyle='--', color = 'k')
        ...
        axins.annotate('', xy=(df['Vth_down'].iloc[0], df['Ith'].iloc[0]/width*1e6), xytext=(df['Vth_up'].iloc[0], df['Ith'].iloc[0]/width*1e6),
               arrowprops=dict(arrowstyle='<->', color='black'))
        axins.text(0.5*(df['Vth_down'].iloc[0] + df['Vth_up'].iloc[0]), df['Ith'].iloc[0]/width*1.05e6, r'$V_\mathsf{H}$', fontsize=8, va='bottom',ha='center')
        plt.savefig(script_dir+"/figures/hysteresis_IdVg_example.pdf", bbox_inches="tight", transparent=True)
        plt.close()
    ...
    if 1: # Id vs t Hysteresis
        ...
        # Inset
        axins = ax.inset_axes([0.35, 0.35, 0.4, 0.3])
        axins.tick_params(axis='both',
            which='both',
            bottom=False,
            top=False,
        ...
        #ax.set_title(r'Hysteresis $I_\mathsf{D}-V_\mathsf{G}$ curve')
        ax.text(0.2, df['Ith'].iloc[0]/width*1e6*1.1, r'$V_\mathsf{th,up}$', fontsize=6, va='bottom', ha='center')
        ax.text(0.835, df['Ith'].iloc[0]/width*1e6*1.1, r'$V_\mathsf{th,down}$', fontsize=6, va='bottom', ha='center')
        ax.axhline(df['Ith'].iloc[0]/width*1e6, linestyle='--', color = 'k')
        axins.axhline(df['Ith'].iloc[0]/width*1e6, linestyle='--', color = 'k')
        current_max = np.max(np.array(Id)/width*1e6)
        current_decade = current_max / 10
        ...
        axins.text(0.72, 4.5e-3, r'$r_\mathsf{sweep}^{\min}$', fontsize=6, va='center', ha='right')
        axins.text(0.79, 4.5e-3, r'$r_\mathsf{sweep}^{\max}$', fontsize=6, va='center', ha='left')
        # Axis 2 texts
        ...
        plt.savefig(script_dir+"/figures/hysteresis_time_example.pdf", bbox_inches="tight", transparent=True)
        plt.close()
    ...
    if 1: # Hysteresis Vth and DeltaVth vs sweep rate (one figure version)
        ###############################################################################################
        df = pd.read_csv(os.path.join(data_folder,'TUWien_planar_hbn-encapsulated','hyst_TUWien_planar_hbn-encapsulated_nMOS.csv'))
        df = df[(df['dut'] == '1A13t1') & (df['temp'] == '300K') & (df['sample'] == 4)]
        ...
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(2.2, 2.5), sharex=True)
        ...
        ax2.set_xlabel(r'Sweep Rate, $r_\mathsf{sweep}$ [V/s]', fontsize=8)
        ax2.set_xscale('log')
        ...
        # Save figure
        script_dir = os.path.dirname(os.path.abspath(__file__))
        plt.savefig(os.path.join(script_dir, "figures", "hysteresis_Vth_DeltaVth_vs_freq.pdf"), bbox_inches="tight", transparent=True)

        plt.close()

    if 0: # Plot BTI IdVg Stress
        df = pd.read_csv("data/OTF_raw_data_TUWien_planar_hbn-encapsulated.csv")
        df = df[(df['dut'] == '2A13t1') & (df['T'] == '300K') & (df['sample'] == 1) & (df['cycle'] == 5) 
        ...
        plt.title(r'$I_\mathsf{D}-V_\mathsf{G}$ curves during stress')
        plt.savefig(script_dir+"/figures/OTF_IdVg_stress.pdf", bbox_inches="tight")
        plt.close()
    ...
    if 0: # Plot hysteresis DeltaVth vs freq different Vd
        ...
        for idx, key in enumerate(all_keys):
            ...
            if key in df_groups:
                ...
                ax.plot(subset['freq'], subset['DeltaVth_fit'], linestyle='-', markersize=8,color=Vd_color(Vd))
        
        ax.axhline(0, linestyle='--', color = 'k')
        # Remove duplicate legend entries
        handles, labels = ax.get_legend_handles_labels()
        by_label = dict(zip(labels, handles))
        ax.legend(by_label.values(), by_label.keys(), fontsize=textSizeLegend, loc='upper right', framealpha=0.9, title = r'$V_D$')
        ...
        plt.savefig(script_dir+"/figures/hysteresis_DeltaVth_vs_freq_differentVd.pdf", bbox_inches="tight", transparent=True)
        plt.close()

        # Plot hysteresis DeltaVth vs freq comparisons
        df = pd.read_csv("data/hyst_TUWien_planar_hbn-encapsulated.csv")
        df = df[~(df['dut'].isin(['2A9t1','1A15t1'])) & (df['precondition'] == False)]
        fig, ax = plt.subplots(figsize=(10, 10))
        ...
        plt.savefig(script_dir+"/figures/hysteresis_DeltaVth_vs_freq_differentVd_duts.pdf", bbox_inches="tight", transparent=True)
        plt.close()
        ...
        # Second legend for DUTs
        ax.legend(dut_handles, dut_labels, fontsize=textSizeLegend, 
              loc='lower right', framealpha=1,bbox_to_anchor=(1.85, 0))
        ...
        device_text = rf'Device {dut}' +'\n'+ r'$T$ = 300 K' + '\n' + rf'$E_{{od}} = {df["Eod"].iloc[0]:.2f}$ MV/cm'
        ax.text(0.05, 0.95, device_text, transform=ax.transAxes,
        fontsize=22, verticalalignment='top',
        bbox=dict(boxstyle='round', facecolor='white', alpha=0.0))
        
        plt.title('Adjusting the voltage ranges')
        plt.savefig(script_dir+"/figures/hysteresis_DeltaVth_vs_freq_differentVd_range.pdf", bbox_inches="tight", transparent=True)
        plt.close()

        ##### Plot BTI OTF DeltaVth vs different VgStress
    ...
    if 0: # Plot BTI MSM DeltaVth all duts vs different VgStress
        df = pd.read_csv("data/MSM_Vth_TUWien_planar_hbn-encapsulated.csv")
        ...
        for i,VgStress in enumerate(VgStress_array[::-1]):
            ...
            df_fit_Vstr = df_fit[(df_fit['VgStress']==VgStress)]
            df_groups_Vstr = dict(tuple(df_Vstr.groupby(['batch','dut','sample'])))
            df_fit_groups_Vstr = dict(tuple(df_fit_Vstr.groupby(['batch','dut','sample'])))
            for idx, key in enumerate(all_keys):
                marker = markers[idx % len(markers)]
                if key in df_groups_Vstr:
                    subset = df_groups_Vstr[key]
                    ax.plot(subset['tRec'], subset['Vth_t'] - subset['Vth_initial'],
                        marker=marker, linestyle=' ', markersize=10,
                        markeredgecolor="#13073A", markeredgewidth=2,
                        markerfacecolor=device_colors[key],label=rf'$W/L$ = {subset['width'].iloc[0]}/{subset['length'].iloc[0]}; Array {subset['array'].iloc[0]}; Meas {subset['sample'].iloc[0]}; $E_{{od,str}}$ = {subset['Eod_str'].iloc[0]:.2f} MV/cm')

                if key in df_fit_groups_Vstr:
                    subset_fit = df_fit_groups_Vstr[key]
                    # plot fits with same color but different line/marker style (no duplicate label)
                    ax.plot(subset_fit['tRec'], subset_fit['Vth_fit'] - subset['Vth_initial'], linestyle='-', markersize=8,color=device_colors[key])
            if i == 0:
                ylim = ax.get_ylim()
            else:
                ax.set_ylim(ylim)
            ...
            ax.set_ylabel(r'$\Delta V_{\mathsf{th}}$ [V]', fontsize=textSize)
            ax.set_xscale('log')
            ax.legend(fontsize=textSizeLegend, loc='upper left', bbox_to_anchor=(0.85, 1), framealpha=0.9)
            plt.savefig(script_dir+f"/figures/MSM_DeltaVth_duts_{VgStress}.pdf", bbox_inches="tight", transparent=True)
            plt.close()
